...
export default function AdminParticipantsPage({ params }) {
  ...
  const loadParticipants = async () => {
    try {
      ...
      if (Array.isArray(pRes.data)) setViewMode("teams");
  ...
  const filtered = useMemo(() => {
    ...
    if (data?.participants) {
      return data.participants.filter((p) => {
        ...
        const hay = `${p.userId?.name || ""} ${p.inGameName || ""} ${p.bgmiId || ""}`.toLowerCase();
        return hay.includes(term);
      });
    }

    return [];
  }, [data, viewMode, searchTerm, filterStatus]);

  const totals = useMemo(() => {
    if (!data) return { teams: 0, players: 0, paid: 0, pending: 0 };

    if (viewMode === "teams" && Array.isArray(data)) {
      const teams = data.length;
      const players = data.reduce((sum, t) => sum + 1 + (t.members?.length || 0), 0);

      const paid = data.filter((t) => t.paymentStatus === "paid").length;
      const pending = data.filter((t) => t.paymentStatus === "pending").length;

      return { teams, players, paid, pending };
    }

    if (data?.participants) {
      const players = data.participants.length;
      const paid = data.participants.filter((p) => p.paymentStatus === "paid").length;
      const pending = data.participants.filter((p) => p.paymentStatus === "pending").length;

      return { teams: 0, players, paid, pending };
    }

    return { teams: 0, players: 0, paid: 0, pending: 0 };
  }, [data, viewMode]);

  if (!id) {
    return (
      <Card className="p-8 text-center">
        <div className="text-lg font-bold text-slate-800">Invalid Tournament</div>
        <div className="text-slate-600 mt-1">Missing tournament id.</div>
      </Card>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-3">
          ...
          <a className="btn btn-primary flex items-center gap-2" href={`/tournaments/${id}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="p-4">
          <div className="flex items-center justify-between">
            ...
            <div className="h-12 w-12 rounded-xl bg-gradient-to-br from-blue-100 to-blue-50 flex items-center justify-center">
              {viewMode === "teams" ? <GiTeamIdea className="w-6 h-6 text-blue-600" /> : <FiUser className="w-6 h-6 text-blue-600" />}
            </div>
          </div>
        </Card>
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-2xl font-bold text-slate-800">{totals.players}</div>
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-2xl font-bold text-slate-800">{totals.paid}</div>
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-2xl font-bold text-slate-800">{totals.pending}</div>
      ...
      <Card className="p-4">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1">
            <Input
              placeholder="Search by name, BGMI ID, team..."
              value={searchTerm}
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm text-slate-600">
              Showing {filtered.length} of {data.length} teams
            </div>
          </div>
      ...
      {viewMode === "individuals" && data?.participants && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm text-slate-600">
              Showing {filtered.length} of {data.participants.length} participants
            </div>
          </div>
